...
class WarpjsPlugin {
    ...
    get Module() {
        ...
        // Don't return anything if it's not a class, so "new" will not be
        // called.
    }
    ...
    register(warpCore, app, baseUrl, staticUrl) {
        ...
        const pluginApp = this.Module
            ? new this.Module(this.config, warpCore, this.type).app
            : this.module(this.config, warpCore, this.Persistence)
        ;
